...
def write_dataframe_as_table(ws, df, sheet_title):
    ...
    for row_idx, row in enumerate(df.itertuples(index=False), start=2):
        for col_idx, value in enumerate(row, start=1):
            ...
            cell.font = data_font
            # Alternar color de fila: blanco y gris muy claro
            if row_idx % 2 == 0:
...
def generate_excel_report(presentacion: pd.DataFrame, codigos_df: pd.DataFrame, 
    ...
    write_dataframe_as_table(ws_presentacion, presentacion, "PRESENTACION")
    
    # Formato especial para % ASIGNADO A PROYECTOS y Suma
    idx_pct = presentacion.columns.get_loc("% ASIGNADO A PROYECTOS") + 1
    idx_suma = presentacion.columns.get_loc("Suma") + 1
    
    for row in ws_presentacion.iter_rows(min_row=2, max_row=ws_presentacion.max_row):
        for cell in row:
